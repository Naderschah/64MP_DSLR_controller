#= 
Entire file taken from https://github.com/seung-lab/Alembic/blob/master/src/math/convolve_inplace.jl#L520

commented out convolve and cumsum2 as they are never used and caused an error

also immutable becomes struct as this is for v0.6 and im running 1.9.2 -- lets hope it actually works

Added broadcast = in clean!

in convolve_Float64_planned changed A_mul_B! to standard matrix multiplaction * as these should accomplish the same thing

and added broadcast assignment in normxcorr2_preallocated
...
function convolve_Float64_planned(A,B; flip = true, factorable = nothing, crop = :valid, padding = :none, ce = get_convolveenv(A, B; crop = crop, padding = padding))
	...
	if padding == :mean
	  @fastmath @inbounds ce.intermediate_A[:] = mean(A)
        ...
        #= Original implementation for matrix multiplaction --> dropped after v 1.0 
        ...
        Below is rewritten --> matrix multiplaction is the default multiplaction behavior now
        =#
        @fastmath ce.complex_intermediate_A[:] = ce.rfft_plan * ce.intermediate_A;
        @fastmath ce.complex_intermediate_B[:] = ce.rfft_plan * ce.intermediate_B;
        @fastmath elwise_mul!(ce.complex_intermediate_A, ce.complex_intermediate_B);
        @fastmath ce.intermediate_A[:] = ce.irfft_plan * ce.complex_intermediate_A
...
function normxcorr2_preallocated(template,img; shape = "valid", highpass_sigma = 0)
    ...
    @fastmath @inbounds LL=UnitRange{Int64}[(n1+1:m1+1),(n2+1:m2+1)]
